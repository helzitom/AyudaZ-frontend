...
import { auth } from '../../firebase'; // ← necesario para obtener el token
...
const Register = () => {
    ...
    const formAyudado = useForm({
        defaultValues: {
            ...
            nombreConyuge: '', fechaNacimientoConyuge: '', lugarNacimientoConyuge: '', cantidadIntegrantes: 1
        }
    });
    ...
    const submitVoluntario = async (data) => {
        ...
        try {
            ...
            // 2. Obtener el usuario actual y su token
            const firebaseUser = auth.currentUser;
            if (!firebaseUser) throw new Error('No se pudo obtener el usuario de Firebase');
            const token = await firebaseUser.getIdToken();
            // 3. Llamar al backend para asignar rol de voluntario
            ...
            alert('Error al registrar voluntario: ' + mensaje);
        } finally {
            setLoading(false);
        }
    };
    ...
    const submitAyudado = async (data) => {
        ...
        try {
            ...
            console.error(error);
            const mensaje = error.response?.data || error.message;
            alert('Error al solicitar ayuda: ' + mensaje);
        } finally {
            setLoading(false);
        }
    };
                    ...
                    <div className="animate-slideIn" style={{
                        ...
                        <form onSubmit={formVoluntario.handleSubmit(submitVoluntario)} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                            ...
                            <Campo label="Correo electrónico" error={formVoluntario.formState.errors.email?.message}>
                                <input type="email" placeholder="tucorreo@ejemplo.com" {...formVoluntario.register('email', { required: 'El correo es requerido' })} style={inputStyle} />
                            </Campo>
                            <Campo label="Contraseña" error={formVoluntario.formState.errors.password?.message}>
                                <input type="password" placeholder="******" {...formVoluntario.register('password', { required: 'La contraseña es requerida', minLength: { value: 6, message: 'Mínimo 6 caracteres' } })} style={inputStyle} />
                            </Campo>
                            <Campo label="Confirmar contraseña" error={formVoluntario.formState.errors.confirmPassword?.message}>
                                <input type="password" placeholder="******" {...formVoluntario.register('confirmPassword', { required: 'Confirma tu contraseña' })} style={inputStyle} />
                            </Campo>
                    ...
                    <div className="animate-slideIn" style={{
                        ...
                        <form onSubmit={formAyudado.handleSubmit(submitAyudado)} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                            ...
                            <Campo label="Correo electrónico" error={formAyudado.formState.errors.email?.message}>
                                <input type="email" placeholder="tucorreo@ejemplo.com" {...formAyudado.register('email', { required: 'El correo es requerido' })} style={inputStyle} />
                            </Campo>
                            <Campo label="Contraseña" error={formAyudado.formState.errors.password?.message}>
                                <input type="password" placeholder="******" {...formAyudado.register('password', { required: 'La contraseña es requerida', minLength: { value: 6, message: 'Mínimo 6 caracteres' } })} style={inputStyle} />
                            </Campo>
                            <Campo label="Confirmar contraseña" error={formAyudado.formState.errors.confirmPassword?.message}>
                                <input type="password" placeholder="******" {...formAyudado.register('confirmPassword', { required: 'Confirma tu contraseña' })} style={inputStyle} />
                            </Campo>
